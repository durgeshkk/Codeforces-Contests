...
void recur(ll i,ll n,ll len){
    ...
    if(2*i+1 < n){
        store[2*i+1] = ceil(1.0*len/2);
        recur(2*i+1,n,ceil(1.0*len/2));
    }
...
void solve()
{
    ...
    // Count the subaary with length 1
    for(ll i = 0;i<n;++i){
        if(pref[i] == 0){++ans;}
    }
    ...
    show(tree);
    // Pass 2*n for recursion to traverse treenode
    store[0] = n;
    recur(0,2*n,n);
    show(store);

    for(ll i = 1;i<2*n;++i){
        if(tree[i] == store[i-1]){++ans;}
    }cout<<ans<<endl;*/
}
